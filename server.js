...
app.get('/', async (req, res) => {
  try {
    ...
    // Render dashboard with employees and error message
    res.render('index', { employees: enrichedEmployees, error: errorMessage });
  } catch (error) {
    // Handle errors by rendering with empty array
    console.error('Error loading dashboard:', error);
    res.render('index', { employees: [], error: 'Failed to load employee data' });
  }
});
 ...
 * Validates input, creates employee, saves to data store, and redirects to dashboard
 */
app.post('/add', upload.single('profileImage'), async (req, res) => {
  try {
    // Extract name, department, basicSalary from request body
    const { name, department, basicSalary, selectedAvatar } = req.body;
    ...
    // Create employee object with trimmed values and parsed salary
    const newEmployee = {
      id: id,
      name: name.trim(),
 ...
 * Finds employee by ID and renders the edit form with employee data
 */
 ...
 * Validates input, updates employee data, saves to data store, and redirects to dashboard
 */
app.post('/edit/:id', async (req, res) => {
  try {
    ...
    // Update employee fields (preserve ID)
 ...
 * Removes employee from data store and redirects to dashboard
 */
